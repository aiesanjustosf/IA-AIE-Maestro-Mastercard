...
PAGE_ICON  = "logo_aie.png"  # podés reemplazar por tu logo real
MAX_MB     = 50

st.set_page_config(page_title=APP_TITLE, page_icon=PAGE_ICON if os.path.exists(PAGE_ICON) else None, layout="centered")
...
if st.button("Procesar y generar informe") and pdf_file is not None:
    ...
    if size_mb > MAX_MB:
        ...
        with st.spinner("Procesando..."):
            # 1) Extracción y sumatoria (criterios Maestro/Mastercard)
            resumen = extract_resumen_from_bytes(pdf_file.getvalue())

            # 2) Ocultar solo la fila '-IVA (21% en Débitos al Comercio)' si existiera
